...
public class Mysql2OracleDatabaseStructureConvertor implements DatabaseStructureConvertor {

	private JavaTypeMapping oracleJavaType = new OracleJavaTypeMapping();
	...
	public String convertTable(Table table) {
		...
		{
			for (Field field : table.getFields()) {
				if ("Y".equals(field.getIsIndex()) && !"Y".equals(field.getIsPrimaryKey())
						&& !"Y".equals(field.getIsUnique())) {
	...
	public String convertColumn(Table table, Field field) {
				...
				+ "{columnName} {type}{lenScale} {notNull} {pk} {unique}";
		{
			...
			String type = StringUtils.upperCase(
					oracleJavaType.getDbType(mysqlJavaType.get(field.getType(), field.getLength(), field.getScale())));
			String lenScale = "";
			{
	...
	public String convertIndex(Table table, Field field) {
		/*CREATE INDEX AUTH_ORG_MENU_UPDATE_TIME ON AUTH_ORGANIZATION_MENU(UPDATE_TIME);*/
		String index = "CREATE INDEX IX_{indexName} ON {tableName}({column});";
		return StringHelper.replaceBlock(index,
				MapHelper.toMap( /**/
						"indexName",
						StringUtils.substring(StringUtils.upperCase(field.getName()) + "_" + StringHelper.uuid(), 0,
								26), /**/
				"tableName", StringUtils.upperCase(table.getName()), /**/
	...
	public String convertDefaultValue(Table table, Field field) {
		/*ALTER TABLE AUTH_ORGANIZATION_USER MODIFY UPDATE_TIME DEFAULT systimestamp;*/
		String defaultValue = "ALTER TABLE {tableName} MODIFY {columnName} DEFAULT {defaultValue};";
		...
		return StringHelper.replaceBlock(defaultValue,
				MapHelper.toMap( /**/
						...
						"columnName", StringUtils.upperCase(field.getName()), /**/
						"defaultValue",
						isString ? ("'" + field.getDefaultValue() + "'")
								: isDate ? ("to_date('" + field.getDefaultValue() + "', 'yyyy-mm-dd hh24:mi:ss')")
	...
	public static void main(String[] args) {
		try {
			InputStream is = new FileInputStream(
					"/Users/zhaochen/dev/workspace/cocoa/foss-qdp-project/src/main/resources/database/002-qdp-core-table-mysql.sql");
