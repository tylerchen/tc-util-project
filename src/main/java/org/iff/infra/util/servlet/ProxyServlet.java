...
public class ProxyServlet extends HttpServlet {
	...
	// add by tylerchen
	protected Map<String, String> initParameterMap = new LinkedHashMap<String, String>();

	// add by tylerchen END
	@Override
		...
		{//add by tylerchen
			...
			while (names.hasMoreElements()) {
				...
				initParameterMap.put(
						name,
						StringHelper.replaceBlock(value,
	...
	protected void service(HttpServletRequest servletRequest, HttpServletResponse servletResponse)
		...
		try {
			...
			if ("true".equals(initParameterMap.get("cached"))) {//add cache by tylerchen
				...
				response.setDateHeader("Expires", date.getTime() * 100); //Expires:过时期限值 
