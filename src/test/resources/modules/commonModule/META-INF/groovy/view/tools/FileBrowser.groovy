...
class FileBrowserAction{
	...
	def private parseRequest(){
		...
		restrict_path=org.iff.infra.util.moduler.TCApplication.me().getProp('tc_file_browser_path') ?: '$$'
	...
	def index(){
		...
		def contextPath=parse.contextPath, actionContext=parse.actionContext, op=parse.op, parent=parse.parent, path=parse.path, content=parse.content
		if(!op){
			list(parse)
		}else{
			if("View"==op){
	...
	def private edit(parse){
		...
		build.html(xmlns:"http://www.w3.org/1999/xhtml"){
			head(){
				 ...
				 [cm,extCM].each{cfg->
					 cfg.css.each{
						 link(type:"text/css", href:"${contextPath}/js/codemirror/${it}", rel:"stylesheet")
					 }
				 }
				 [cm,extCM].each{cfg->
					 cfg.js.each{
						 script('',type:"text/javascript", src:"${contextPath}/js/codemirror/${it}")
					 }
				 }
			}
			body(){
	...
	def private save(parse){
		...
		addUrlParam('parent',parent).addUrlParam('path',path).addUrlParam('op','Edit').redirect("${contextPath}/${actionContext}")
	}
	def private upload(parse){
		...
		addUrlParam('parent',parent).redirect("${contextPath}/${actionContext}")
	}
